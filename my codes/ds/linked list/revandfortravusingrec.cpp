...
int main()
{
  ...
  while(1)
  {
    cout<<"1-creating 2-traverse 3-insert 4-delete 5-traverse using recursion 6-reverse traversing 7-exit"<<endl;
    cin>>x;
    switch(x)
    {
      case 1:
  ...
  void travrec(node*temp)
  {
    cout<<(temp->a)<<endl;
    if(temp->link!=NULL)
    travrec(temp->link);
  }
void revtravrec(node*temp)
  { if(temp->link!=NULL)
    revtravrec(temp->link);
    cout<<(temp->a)<<endl;
  }
